...
class AuthDAO:
    ...
    def register(self, email, plain_password, name):
        ...
        def create_user(tx, email, encrypted, name):
            ...
            return tx.run(query, email=email, encrypted=encrypted, name=name).single()
    ...
    def _generate_token(self, payload):
        ...
        payload["iat"] = str(iat)
        payload["nbf"] = str(iat)
        payload["exp"] = str(iat) + str(current_app.config.get('JWT_EXPIRATION_DELTA'))
    ...
    """
    This method will attempt to decode a JWT token
    """
    # tag::decode[]
